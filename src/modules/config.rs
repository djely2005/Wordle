use crate::modules::revelation::{LetterRevelation, WordRevelation};
use rand::rng;
...
impl Config {
    ...
    fn get_correct_revelations(&self, guessed_word: &str, true_word: &mut Vec<u8>) -> Vec<LetterRevelation> {
        guessed_word
            ...
            .iter()
            .enumerate()
            .map(|c| LetterRevelation::get_correct(true_word, c.1, c.0))
    ...
    fn get_incorrect_revelation(&self, guessed_word: &str, true_word: &mut Vec<u8>, correct_revelations: &Vec<LetterRevelation>) -> Vec<LetterRevelation> {
        guessed_word
            ...
            .map(|c| LetterRevelation::get_incorrect(true_word, c.1, c.0))
            .collect()
    }
